...
class FlowmatchingActionHead(nn.Module):
    def __init__(
        ...
        # Prefer values from the provided config when available (e.g. checkpoint's run config),
        # otherwise fall back to the global constants detected at import time.
        self.action_dim = getattr(config, "action_dim", None) or ACTION_DIM
        self.proprio_dim = getattr(config, "proprio_dim", None) or PROPRIO_DIM
        self.action_horizon = getattr(config, "action_horizon", None) or NUM_ACTIONS_CHUNK
