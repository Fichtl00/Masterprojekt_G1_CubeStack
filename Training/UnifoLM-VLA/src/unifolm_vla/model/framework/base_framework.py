...
class baseframework(nn.Module):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        # Load the checkpoint early so we can infer architecture dimensions from the saved weights.
        # This protects against stale config.yaml values that no longer match the checkpoint.
        model_state_dict = torch.load(pretrained_checkpoint, map_location="cpu")
        action_decoder_weight = model_state_dict.get("action_model.action_decoder.layer2.weight")
        action_encoder_weight = model_state_dict.get("action_model.action_encoder.layer1.weight")
        if action_decoder_weight is not None:
            inferred_action_dim = int(action_decoder_weight.shape[0])
        else:
            inferred_action_dim = None
        if action_encoder_weight is not None:
            inferred_proprio_dim = int(action_encoder_weight.shape[1])
        else:
        ...
        # Apply explicit overrides first, then checkpoint-derived dimensions.
        for key, value in kwargs.items():
            setattr(model_config.framework.action_model, key, value)
        if inferred_action_dim is not None:
            model_config.framework.action_model.action_dim = inferred_action_dim
        if inferred_proprio_dim is not None:
            model_config.framework.action_model.proprio_dim = inferred_proprio_dim
            model_config.framework.action_model.state_dim = inferred_proprio_dim
