...
class VLATrainer(TrainerUtils):
    ...
    def _init_wandb(self):
        ...
        if self.accelerator.is_main_process:
            ...
            # No native TensorBoard support in this trainer (only offline wandb) -- add it
            # alongside wandb for local monitoring, analogous to gr00t_finetuning_anleitung.md Schritt 5.
...
def main(cfg) -> None:
    ...
    # Freeze modules BEFORE building the optimizer/param groups: DeepSpeed's BF16Optimizer
    # filters each param group by `requires_grad` internally (bf16_optimizer.py::_setup_for_real_optimizer),
    # so a group that is entirely frozen crashes with `torch.cat(): expected a non-empty list
    # of Tensors` if it was still built from the model while unfrozen. Freezing here (instead
    # of later in VLATrainer.prepare_training) ensures build_param_lr_groups already excludes it.
