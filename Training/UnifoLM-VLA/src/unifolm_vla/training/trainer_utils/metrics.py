...
def build_param_lr_groups(model, cfg):
    ...
    for module_name, lr in lr_cfg.items():
        ...
        try:
            ...
            # Frozen params (requires_grad=False, e.g. via trainer.freeze_modules) must be
            # excluded here: DeepSpeed ZeRO-2 crashes with `torch.cat(): expected a
            # non-empty list of Tensors` when a param group never produces any gradients.
            params = [p for p in module.parameters() if p.requires_grad]
            used_params.update(id(p) for p in module.parameters())
            if params:
