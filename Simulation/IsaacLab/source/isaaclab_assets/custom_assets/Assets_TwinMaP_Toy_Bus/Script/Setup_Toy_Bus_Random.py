...
# Pfade für Files
# ----------------------------------------
BODY_USD_PATH = "/home/omniverse-2/Documents/Toy_Bus/Body.usd"
SCHRAUBE1_USD_PATH = "/home/omniverse-2/Documents/Toy_Bus/Schraube_1.usd"
...
RANDOM_XY_MIN = -0.25
RANDOM_XY_MAX = 0.25
BODY_MIN_DISTANCE = 0.28
SCREW_MIN_DISTANCE = 0.12
MAX_RANDOM_ATTEMPTS = 300
...
def get_or_create_translate_and_rotate_ops(prim):
...
def set_translation_and_rotation(prim, x_value, y_value, z_value):
    translate_op, rotate_op = get_or_create_translate_and_rotate_ops(prim)
    translate_op.Set(Gf.Vec3d(x_value, y_value, z_value))
    rotate_op.Set(Gf.Vec3f(-90.0, 0.0, 0.0))


def is_far_enough(x, y, placed_positions, min_distance):
    for px, py, required_distance in placed_positions:
        distance = math.sqrt((px - x) ** 2 + (py - y) ** 2)
        if distance < max(min_distance, required_distance):
            return False
    return True


def get_random_non_overlapping_position(placed_positions, min_distance):
    for _ in range(MAX_RANDOM_ATTEMPTS):
        x = random.uniform(RANDOM_XY_MIN, RANDOM_XY_MAX)
        y = random.uniform(RANDOM_XY_MIN, RANDOM_XY_MAX)

        if is_far_enough(x, y, placed_positions, min_distance):
            return x, y

    raise RuntimeError("Keine kollisionsfreie Random-Position gefunden. Bereich zu klein oder Mindestabstand zu groß.")
...
def setup_part(usd_path, prim_path, mass, x_value, y_value):
    ...
    set_translation_and_rotation(root_prim, x_value, y_value, Z_OFFSET)
    ...
    print(f"{prim_path} -> position=({x_value:.3f}, {y_value:.3f}, {Z_OFFSET:.3f}), SDF applied to {len(collision_meshes)} mesh prim(s)")
...
body_x, body_y = get_random_non_overlapping_position(placed_positions, BODY_MIN_DISTANCE)
placed_positions.append((body_x, body_y, BODY_MIN_DISTANCE))
setup_part(BODY_USD_PATH, BODY_PRIM_PATH, mass=1.0, x_value=body_x, y_value=body_y)

screw_configs = [
    (SCHRAUBE1_USD_PATH, SCHRAUBE1_PRIM_PATH),
    (SCHRAUBE2_USD_PATH, SCHRAUBE2_PRIM_PATH),
    (SCHRAUBE3_USD_PATH, SCHRAUBE3_PRIM_PATH),
    (SCHRAUBE4_USD_PATH, SCHRAUBE4_PRIM_PATH),
    (SCHRAUBE5_USD_PATH, SCHRAUBE5_PRIM_PATH),
    (SCHRAUBE6_USD_PATH, SCHRAUBE6_PRIM_PATH),
    (SCHRAUBE7_USD_PATH, SCHRAUBE7_PRIM_PATH),
    (SCHRAUBE8_USD_PATH, SCHRAUBE8_PRIM_PATH),
    (SCHRAUBE9_USD_PATH, SCHRAUBE9_PRIM_PATH),
]

for usd_path, prim_path in screw_configs:
    x, y = get_random_non_overlapping_position(placed_positions, SCREW_MIN_DISTANCE)
    placed_positions.append((x, y, SCREW_MIN_DISTANCE))
    setup_part(usd_path, prim_path, mass=0.1, x_value=x, y_value=y)