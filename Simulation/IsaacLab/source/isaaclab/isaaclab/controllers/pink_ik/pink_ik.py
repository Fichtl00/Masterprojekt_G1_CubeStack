...
# qpsolvers 4.12.0 passes `primal_start` to DAQP, but the DAQP build shipped
# with Isaac Sim 5.1.0 / Isaac Lab 2.3.x exposes an older `daqp.solve`
# signature that does not accept that keyword.
# Wrap the backend once at import time so Pink IK can keep using the DAQP solver.
try:
    import inspect

    import daqp

    if "primal_start" not in inspect.signature(daqp.solve).parameters:
        _daqp_solve = daqp.solve

        def _daqp_solve_compat(*args, primal_start=None, **kwargs):
            return _daqp_solve(*args, **kwargs)

        daqp.solve = _daqp_solve_compat
except Exception:
    pass
